...
/**
 * Permanent planner-pipeline ownership guard (ADR-0051..0055, ratified linear chain). The only permitted chain is
 * `Marketing -> Content -> SEO -> Social -> Analytics -> Campaign -> Growth Workflows`: each subsystem depends solely on
 * its immediate predecessor, so the further-upstream context is reached transitively. No fan-in edges, no reverse edges,
 * no cycles. This guard reads every stage's declared runtime dependencies (and this package's own source) and fails the
 * build on any forbidden edge. It is a permanent architectural regression test.
 */
...
describe('planner pipeline ownership guard (chain Marketing -> ... -> Campaign -> Growth Workflows)', () => {
  ...
  it('this package imports only its predecessor (Campaign) among the chain packages in source', () => {
    const srcDir = fileURLToPath(new URL('../src/', import.meta.url));
    const text = readdirSync(srcDir)
      .filter((name) => name.endsWith('.ts'))
      .map((name) => readFileSync(join(srcDir, name), 'utf8'))
      .join('\n');
    expect(text).toContain(pkgName('campaign-intelligence'));
    for (const dir of [
      'marketing-intelligence',
      'content-intelligence',
      'seo-intelligence',
