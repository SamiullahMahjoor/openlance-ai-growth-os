...
- **Production import enforcement:** dependency-cruiser resolves bare workspace specifiers (`@openlance/aios-*`) to each package's source barrel (`tsconfig.depcruise.json`, ADR-0019), so every rule fires against the exact import syntax production code uses -- **bare package imports, relative imports, namespace imports, and workspace imports** -- and not only relative filesystem imports. No supported import form bypasses enforcement. `scripts/arch-regression.mjs` (in `validate` and CI) proves each rule rejects a forbidden production import and accepts a legal one.
...
**Acceptance criteria.** Introducing a cycle, a forbidden reference, a wrong-direction edge, or a deep import fails CI; the enforced graph is identical to `dependency-map.md`; no path exists to merge a violating change. Enforcement applies equally to bare package imports, relative imports, namespace imports, and workspace imports; no supported import form bypasses it, and the architectural regression suite proves every rule rejects a forbidden production import.
...
## Constitutional Rule, Production Import Enforcement

**Statement.** Architectural enforcement SHALL operate against the exact import syntax used by production code.

Every dependency rule MUST be validated using production package specifiers (for example `@openlance/aios-*`) rather than only relative filesystem imports. A rule that cannot reject an illegal production import is considered **non-functional**.

Every architectural rule MUST include permanent regression tests proving:

- forbidden layer inversions fail,
- forbidden namespace edges fail,
- forbidden substrate to namespace edges fail,
- forbidden cycles fail,
- forbidden deep imports fail,
- legal imports succeed.

This requirement is mandatory for every future architectural rule.

**Realization.** dependency-cruiser resolves bare workspace specifiers to source (`tsconfig.depcruise.json`, ADR-0019), so the Rule 1 and Rule 2 rules fire against production bare-import syntax. The regression suite `scripts/arch-regression.mjs` (in `pnpm run validate` and CI) constructs each forbidden and each legal scenario with production specifiers and asserts the exact rule fires or the legal import passes. Adding a new architectural rule requires adding its scenarios to that suite; a rule without passing production-syntax regression scenarios is not considered functional and must not be relied upon.
