 ...
 * Analytics package so either package alone enforces it.
 */
...
describe('planner pipeline ownership guard (ratified chain Marketing -> Content -> SEO -> Social -> Analytics -> Campaign)', () => {
  ...
  it('this package imports only its predecessor (Analytics) among the planners in source', () => {
    const srcDir = fileURLToPath(new URL('../src/', import.meta.url));
    const text = readdirSync(srcDir)
      .filter((name) => name.endsWith('.ts'))
      .map((name) => readFileSync(join(srcDir, name), 'utf8'))
      .join('\n');
    expect(text).toContain(pkgName('analytics-intelligence'));
    for (const dir of [
      'marketing-intelligence',
      'content-intelligence',
      'seo-intelligence',
